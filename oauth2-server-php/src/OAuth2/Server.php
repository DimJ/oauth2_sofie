...
use OAuth2\ResponseType\SofieJwtAccessToken;
    ...
    UserInfoControllerInterface
{
    ...
    /**
     * EXTENSION_FOR_SOFIE_PROJECT
     *
     * @var AS1ThingKey 
     */
    protected $AS1ThingKey;
    ...
    /**
     * Extend the handleTokenRequest function, for the "Sophie" project purposes.
     *
     * @param RequestInterface $request   - Request object to grant access token
     * @param ResponseInterface $response - Response object containing error messages (failure) or access token (success)
     * @return ResponseInterface
     ...
     * @ingroup oauth2_section_4
     */
    public function handleTokenRequestForSophie( string $scenario, RequestInterface $request, ResponseInterface $response = null )
    {
        $this->response = is_null($response) ? new Response() : $response;
        $this->getTokenController()->handleTokenRequest($request, $this->response);
        if( strcmp( $scenario, "Scenario1" ) == 0 )
        {
            $this->addExtraFieldsJWT_scenario1( $this->response );
        }
        else if( strcmp( $scenario, "Scenario2" ) == 0 )
        {
            $this->addExtraFieldsJWT_scenario2( $this->response );
        }
        else if( strcmp( $scenario, "ScenarioDistributed" ) == 0 )
        {
            // to be implemented
        }
        else 
            exit( " - Invalid scenario! " );
    ...
    protected function getAccessTokenResponseType()
    {
        ...
        else if ($this->config['use_jwt_access_tokens_sophie_experiments']) {
            return $this->createSophieJwtAccessTokenResponseType();
        }
     ...
     *
     * @return SophieJwtAccessToken
     * @throws LogicException
     */
    protected function createSophieJwtAccessTokenResponseType()
    {
        if (!isset($this->storages['public_key'])) {
            throw new \LogicException('You must supply a storage object implementing OAuth2\Storage\PublicKeyInterface to use crypto tokens');
        }
        ...
        return new SofieJwtAccessToken($this->AS1ThingKey, $this->storages['public_key'], $tokenStorage, $refreshStorage, $config);
    }
    ...
    /**
     * SOFIE protocol, implement protocol's keys
     * @param AS1ThingKey , a key for symmetric encryption of POP key. 
     * @return publicKey , a key for public-key encryption of POP key
     */
    public function setKeys( $AS1ThingKey, $publicKey = "" )
    {
        $this->AS1ThingKey = trim(file_get_contents($AS1ThingKey));
        
        if( !empty($publicKey) )
            $this->publicKey = trim(file_get_contents($publicKey));
    }

    /**
    * EXTENSION_FOR_SOFIE_PROJECT
    *
    * @param string original response from OAuth2 Server
    * @return the extended response(for the OAuth2/Blockchain )  
    */
    private function addExtraFieldsJWT_scenario1( $originalResponse )
    {
        $extraParameters = array();
        
        $secret = bin2hex(random_bytes(16)); // produce a 32 byte string value
        $pop = bin2hex(random_bytes(16)); // produce a 32 byte string value
        $price = rand(1000000000, 5000000000);
        
        $jwt = new Encryption\Jwt();

        $extraParameters["e_s_token"] =  $this->AES_Encryption( $originalResponse->getParameter("access_token"), $secret );
        $extraParameters["e_thing_pop"] = $this->AES_Encryption( $pop, $this->AS1ThingKey );
        $extraParameters["pop"] = $pop;
    ...
    * @return the extended response(for the OAuth2/Blockchain )  
    */
    private function addExtraFieldsJWT_scenario2( $originalResponse )
    {
        $extraParameters = array();
    ...
    private function AES_Encryption( $plaintext, $key )
    {
    ...
    private function publicKeyEncryption( $plaintext, $publicKey )
    {
